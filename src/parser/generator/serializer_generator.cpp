...
namespace Generator
{
    ...
    SerializerGenerator::SerializerGenerator(std::function<std::string(std::string)> get_include_function) 
            : GeneratorInterface(get_include_function)
    {
        prepareStatus();
    }

    void SerializerGenerator::prepareStatus()
    {
        auto tmpMgr = TemplateManager::getInstance();
        tmpMgr->loadTemplates(single_h_temp_name);
        tmpMgr->loadTemplates(single_cpp_temp_name);
        tmpMgr->loadTemplates(all_h_temp_name);
        tmpMgr->loadTemplates(all_ipp_temp_name);
        return;
    }
    ...
    int SerializerGenerator::generate(std::string path, SchemaMoudle schema)
    {
            ...
            for (int index = 0; index < class_temp->m_base_classes.size(); ++index)
            {
                ...
                if (!include_file.empty())
                {
                    ...
                    if (headfile_path != include_file_base)
                    {
                        ...
                        std::string rela_path = Utils::makeRelativePath(source_root, include_file_base);
                        include_headfiles.push_back(Mustache::data("headfile_item", rela_path));
                    }
                }
            }
        ...
        std::string gen_rela_path = Utils::makeRelativePath(source_root, headfile_path);
